...
class HeaderWrapper(object):
    ...
    def from_header(cls, header):
        return cls(decred_data.hash256(decred_data.block_header_type.pack(header)), header['previous_block'])
...
@defer.inlineCallbacks
def get_height_rel_highest_func(dcrd, factory, best_block_func, net):
    if '\ngetblock ' in (yield deferral.retry()(dcrd.rpc_help)()):
        @deferral.DeferredCacher
        @defer.inlineCallbacks
        def height_cacher(block_hash):
            try:
                x = yield dcrd.rpc_getblock('%x' % (block_hash,))
