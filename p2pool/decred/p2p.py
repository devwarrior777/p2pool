...
class Protocol(p2protocol.Protocol):
    ...
    def connectionMade(self):
        self.send_version(
            version=4, # gf: Current version returned from getinfo is 4 (code says 5)
            services=1,
            time=int(time.time()),
            addr_to=dict(
                services=1,
    ...
    message_tx = pack.ComposedType([
        ('tx', decred_data.tx_type),
    ])
    def handle_tx(self, tx):
        self.factory.new_tx.happened(tx)
    
    message_block = pack.ComposedType([
        ('block', decred_data.block_type),
    ])
    def handle_block(self, block):
        block_hash = decred_data.hash256(decred_data.block_header_type.pack(block['header']))
    ...
    message_headers = pack.ComposedType([
        ('headers', pack.ListType(decred_data.block_type)),
    ])
    def handle_headers(self, headers):
        for header in headers:
            header = header['header']
            self.get_block_header.got_response(decred_data.hash256(decred_data.block_header_type.pack(header)), header)
    ...
    def handle_reject(self,message,ccode,reason):
        print message, ccode, reason
