...
class NewShare(object):
    ...
    share_info_type = pack.ComposedType([
        ...
        ('max_bits', decred_data.FloatingIntegerType()),
        ('bits', decred_data.FloatingIntegerType()),
        ('timestamp', pack.IntType(32)),
    ...
    def generate_transaction(cls, tracker, share_data, block_target, desired_timestamp, desired_target, ref_merkle_link, desired_other_transaction_hashes_and_fees, net, known_txs=None, last_txout_nonce=0, base_subsidy=None):
        ...
        max_bits = decred_data.FloatingInteger.from_target_upper_bound(pre_target3)
        bits = decred_data.FloatingInteger.from_target_upper_bound(math.clip(desired_target, (pre_target3//30, pre_target3)))
        ...
        for tx_hash, fee in desired_other_transaction_hashes_and_fees:
            if tx_hash in tx_hash_to_this:
                ...
                if known_txs is not None:
                    this_size = decred_data.tx_type.packed_size(known_txs[tx_hash])
        ...
        weights, total_weight, donation_weight = tracker.get_cumulative_weights(previous_share.share_data['previous_share_hash'] if previous_share is not None else None,
            ...
            65535*net.SPREAD*decred_data.target_to_average_attempts(block_target),
        )
        ...
        def get_share(header, last_txout_nonce=last_txout_nonce):
            ...
            share = cls(net, None, dict(
                ...
                hash_link=prefix_to_hash_link(decred_data.tx_type.pack(gentx)[:-32-8-4], cls.gentx_before_refhash),
    ...
    def get_ref_hash(cls, net, share_info, ref_merkle_link):
        return pack.IntType(256).pack(decred_data.check_merkle_link(decred_data.hash256(cls.ref_type.pack(dict(
    ...
    def __init__(self, net, peer_addr, contents):
        ...
        merkle_root = decred_data.check_merkle_link(self.gentx_hash, self.merkle_link)
        self.header = dict(self.min_header, merkle_root=merkle_root)
        self.pow_hash = net.PARENT.POW_FUNC(decred_data.block_header_type.pack(self.header))
        self.hash = self.header_hash = decred_data.hash256(decred_data.block_header_type.pack(self.header))
    ...
    def should_punish_reason(self, previous_block, bits, tracker, known_txs):
        ...
        if other_txs is None:
            ...
            all_txs_size = sum(decred_data.tx_type.packed_size(tx) for tx in other_txs)
            if all_txs_size > 1000000:
                return True, 'txs over block size limit'
            
            new_txs_size = sum(decred_data.tx_type.packed_size(known_txs[tx_hash]) for tx_hash in self.share_info['new_transaction_hashes'])
...
class OkayTracker(forest.Tracker):
    def __init__(self, net):
        forest.Tracker.__init__(self, delta_type=forest.get_attributedelta_type(dict(forest.AttributeDelta.attrs,
            work=lambda share: decred_data.target_to_average_attempts(share.target),
            min_work=lambda share: decred_data.target_to_average_attempts(share.max_target),
        )))
        self.net = net
    ...
    def think(self, block_rel_height_func, previous_block, bits, known_txs):
        ...
        if p2pool.DEBUG:
            print 'Desire %i shares. Cutoff: %s old diff>%.2f' % (len(desired), math.format_dt(time.time() - timestamp_cutoff), decred_data.target_to_difficulty(target_cutoff))
            for peer_addr, hash, ts, targ in desired:
                print '   ', None if peer_addr is None else '%s:%i' % peer_addr, format_hash(hash), math.format_dt(time.time() - ts), decred_data.target_to_difficulty(targ), ts >= timestamp_cutoff, targ <= target_cutoff
...
def get_stale_counts(tracker, share_hash, lookbehind, rates=False):
    ...
    for share in tracker.get_chain(share_hash, lookbehind - 1):
        res['good'] = res.get('good', 0) + decred_data.target_to_average_attempts(share.target)
        s = share.share_data['stale_info']
        if s is not None:
            res[s] = res.get(s, 0) + decred_data.target_to_average_attempts(share.target)
    if rates:
...
def get_expected_payouts(tracker, best_share_hash, block_target, subsidy, net):
    weights, total_weight, donation_weight = tracker.get_cumulative_weights(best_share_hash, min(tracker.get_height(best_share_hash), net.REAL_CHAIN_LENGTH), 65535*net.SPREAD*decred_data.target_to_average_attempts(block_target))
...
def get_desired_version_counts(tracker, best_share_hash, dist):
    ...
    for share in tracker.get_chain(best_share_hash, dist):
        res[share.desired_version] = res.get(share.desired_version, 0) + decred_data.target_to_average_attempts(share.target)
    return res

def get_warnings(tracker, best_share, net, dcrd_getinfo, dcrd_work_value):
    res = []
    ...
    if dcrd_getinfo['errors'] != '':
        if 'This is a pre-release test build' not in dcrd_getinfo['errors']:
            res.append('(from dcrd) %s' % (dcrd_getinfo['errors'],))
    
    version_warning = getattr(net, 'VERSION_WARNING', lambda v: None)(dcrd_getinfo['version'])
    if version_warning is not None:
        res.append(version_warning)
    
    if time.time() > dcrd_work_value['last_update'] + 60:
        res.append('''LOST CONTACT WITH BITCOIND for %s! Check that it isn't frozen or dead!''' % (math.format_dt(time.time() - dcrd_work_value['last_update']),))
