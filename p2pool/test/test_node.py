...
class dcrd(object): # can be used as p2p factory, p2p protocol, or rpc jsonrpc proxy
    def __init__(self):
        ...
        self.headers = {0x16c169477c25421250ec5d32cf9c6d38538b5de970a2355fd89: {
            ...
            'bits': decred_data.FloatingInteger(bits=0x1a0513c5, target=0x513c50000000000000000000000000000000000000000000000L),
        }}
    ...
    def rpc_getblocktemplate(self, param):
        if param['mode'] == 'template':
            ...
            result = param['data']
            block = decred_data.block_type.unpack(result.decode('hex'))
            ...
            if block['header']['previous_block'] != self.blocks[-1]:
                return False
            if decred_data.hash256(result.decode('hex')) > block['header']['bits'].target:
                return False
            header_hash = decred_data.hash256(decred_data.block_header_type.pack(block['header']))
        ...
        for i in xrange(100):
            fee = i
            txs.append(dict(
                data=decred_data.tx_type.pack(dict(version=1, tx_ins=[], tx_outs=[dict(value=fee, script='hello!'*100)], lock_time=0)).encode('hex'),
                fee=fee,
            ))
        return {
            "version" : 2,
...
class MiniNode(object):
    ...
    def start(cls, net, factory, dcrd, peer_ports, merged_urls):
        ...
        wb = work.WorkerBridge(node=self.n, my_pubkey_hash=random.randrange(2**160), donation_percentage=random.uniform(0, 10), merged_urls=merged_urls, worker_fee=3, args=math.Object(donation_percentage=random.uniform(0, 10), address='foo', worker_fee=3, timeaddresses=1000), pubkeys=main.keypool(), dcrd=dcrd)
        self.wb = wb
...
class Test(unittest.TestCase):
    ...
    def test_node(self):
        ...
        wb = work.WorkerBridge(node=n, my_pubkey_hash=42, donation_percentage=2, merged_urls=[('http://127.0.0.1:%i' % (mm_port.getHost().port,), '')], worker_fee=3, args=math.Object(donation_percentage=2, address='foo', worker_fee=3, timeaddresses=1000), pubkeys=main.keypool(), dcrd=bitd)
        web_root = resource.Resource()
        worker_interface.WorkerInterface(wb).attach_to(web_root)
