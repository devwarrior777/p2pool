...
class P2PNode(p2p.Node):
    ...
    def handle_bestblock(self, header, peer):
        if self.node.net.PARENT.POW_FUNC(decred_data.block_header_type.pack(header)) > header['bits'].target:
        ...
        def _(share):
            ...
            def spread():
                if (self.node.get_height_rel_highest(share.header['previous_block']) > -5 or
                    self.node.dcrd_work.value['previous_block'] in [share.header['previous_block'], share.header_hash]):
...
class Node(object):
    def __init__(self, factory, dcrd, shares, known_verified_share_hashes, net):
    ...
    def start(self):
        ...
        def handle_header(new_header):
            ...
            if not (self.net.PARENT.POW_FUNC(decred_data.block_header_type.pack(new_header)) <= self.dcrd_work.value['bits'].target):
                return
            dcrd_best_block = self.dcrd_work.value['previous_block']
            if (self.best_block_header.value is None
                or (
                    new_header['previous_block'] == dcrd_best_block and
                    decred_data.hash256(decred_data.block_header_type.pack(self.best_block_header.value)) == dcrd_best_block
                ) # new is child of current and previous is current
                or (
                    decred_data.hash256(decred_data.block_header_type.pack(new_header)) == dcrd_best_block and
                    self.best_block_header.value['previous_block'] != dcrd_best_block
        ...
        def poll_header():
            if self.factory.conn.value is None:
                return
            handle_header((yield self.factory.conn.value.get_block_header(self.dcrd_work.value['previous_block'])))
        self.dcrd_work.changed.watch(lambda _: poll_header())
        ...
        self.get_height_rel_highest = yield height_tracker.get_height_rel_highest_func(self.dcrd, self.factory, lambda: self.dcrd_work.value['previous_block'], self.net)
        
        self.best_share_var = variable.Variable(None)
        self.desired_var = variable.Variable(None)
        self.dcrd_work.changed.watch(lambda _: self.set_best_share())
        self.set_best_share()
        
        # setup p2p logic and join p2pool network
        
        # update mining_txs according to getwork results
        @self.dcrd_work.changed.run_and_watch
        def _(_=None):
            new_mining_txs = {}
            new_known_txs = dict(self.known_txs_var.value)
            for tx_hash, tx in zip(self.dcrd_work.value['transaction_hashes'], self.dcrd_work.value['transactions']):
                new_mining_txs[tx_hash] = tx
                new_known_txs[tx_hash] = tx
            self.mining_txs_var.set(new_mining_txs)
            self.known_txs_var.set(new_known_txs)
        # add p2p transactions from dcrd to known_txs
        ...
        def _(share):
            ...
            if block is None:
                print >>sys.stderr, 'GOT INCOMPLETE BLOCK FROM PEER! %s decred: %s%064x' % (p2pool_data.format_hash(share.hash), self.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
                return
            helper.submit_block(block, True, self.factory, self.dcrd, self.dcrd_work, self.net)
            print
            print 'GOT BLOCK FROM PEER! Passing to dcrd! %s decred: %s%064x' % (p2pool_data.format_hash(share.hash), self.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
            print
    ...
    def set_best_share(self):
        best, desired, decorated_heads, bad_peer_addresses = self.tracker.think(self.get_height_rel_highest, self.dcrd_work.value['previous_block'], self.dcrd_work.value['bits'], self.known_txs_var.value)
    ...
    def get_current_txouts(self):
        return p2pool_data.get_expected_payouts(self.tracker, self.best_share_var.value, self.dcrd_work.value['bits'].target, self.dcrd_work.value['subsidy'], self.net)
